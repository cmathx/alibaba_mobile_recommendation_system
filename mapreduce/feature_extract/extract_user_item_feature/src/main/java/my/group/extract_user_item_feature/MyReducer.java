...
//		long[] first_beh_day = new long[5];
//		long[] final_beh_day = new long[5];
//
		...
		while (values.hasNext()) {
			...
			if (t_hour <= -24)// 不考虑不在训练集里面的行为集合
				...
				if (type == 3)
					is_buy = 1L;
				continue;
			} else if (t_hour == 23 || t_hour == 24 || t_hour <= 17) {
				// 只考虑最近1天浏览过的行为
				if (type == 0)
					is_consider_set = 1L;
			}
...
//			if (t_hour > first_beh_hour[type])
//				first_beh_hour[type] = t_hour;
//			if (t_hour > first_beh_hour[4])
//				first_beh_hour[4] = t_hour;
//
//			if (t_day < final_beh_day[type])
//				final_beh_day[type] = t_day;
//			if (t_day < final_beh_day[4])
//				final_beh_day[4] = t_day;
//			if (t_day > first_beh_day[type])
//				first_beh_day[type] = t_day;
//			if (t_day > first_beh_day[4])
//				first_beh_day[4] = t_day;
//
...
//			result.set(i + 24, final_beh_day[i]);// 用户行为最近时间
//		}
//		for (int i = 0; i < 5; ++i) {
//			if (first_beh_hour[i] == -1L)
//				first_beh_hour[i] = 720L;
//			result.set(i + 19, first_beh_hour[i]);
//			if (first_beh_day[i] == -1L)
//				first_beh_day[i] = 31L;
//			result.set(i + 29, first_beh_day[i]);
//		}
//
//		boolean flag = true;
...
//				interval[i] = 31.0;
//		} else {
//			interval[2] = 1.0 * (Long) contact_day.get(contact_day.size() - 1) - (Long) contact_day.get(0);
//			interval[3] = 0L;
...
//
//		for (int i = 0; i < 6; ++i) {
//			result.set(i + 34, first_last_cnt[i]);
//		}
//		for (int i = 0; i < 4; ++i)
//			result.set(i + 40, interval[i]);
//		for (int i = 0; i < 5; ++i)
//			result.set(i + 44, beh_days[i]);
//		result.set(49, 1.0 * beh_cnt[3] / beh_cnt[4]);
//		result.set(50, 1.0 * beh_days[3] / beh_days[4]);
		result.set(7, is_buy);
		for(int i = 0; i < 5; ++i)
			result.set(2 + i, beh_cnt[i]);

...
'''
def baseUserItemBehavior(user_item_map, ignore_cnt):
    """
    ...
    for file_name in os.listdir(rootDir):
        ...
        for line in open(file_path, 'r'):
            ...
            if user_id in user_item_map and item_id in user_item_map[user_id]:#确保user_id和item_id在待分析的user-item对中
                ...
                user_item_behavior_map[user_id].setdefault(item_id, [0 for i in xrange(0, base_feature_num + 4)])
...
def extractUserItemFeature(user_item_behavior_map):
    ...
    user_item_view_cart_notBuy_feature_num = 2*29
    user_item_rate_feature_num = 8*29
    user_item_feature_num = user_item_view_cart_notBuy_feature_num + user_item_rate_feature_num + 3
    user_item_feature = {}
    for user_id in user_item_behavior_map:
        for item_id in user_item_behavior_map[user_id]:
            ...
            for i in xrange(0, 29):
               ...
               #     user_item_behavior_map[user_id][item_id][i * 4 + 3] == 0:
                    # if i == 0:
                    #     user_item_feature[user_id][item_id][i] = 1
                    # else:
                ...
                # else:
                    # if i == 0:
                    # user_item_feature[user_id][item_id][i] = 0#1
                    # else:
                    #     user_item_feature[user_id][item_id][i] = user_item_behavior_map[user_id][item_id][i - 1]
                for j in xrange(0, 4):
                        ...
                        if user_item_behavior_map[user_id][item_id][4 * k + j] != 0:
                            if user_item_behavior_map[user_id][item_id][4 * 29 + 2] >= 19:
                                if k < 5:
                                    ...
                                    user_item_feature[user_id][item_id][8 * i + j + user_item_view_cart_notBuy_feature_num] -= k / (2 * k + 1)
                            else:
                                if k < 5:
                                    user_item_feature[user_id][item_id][8 * i + 4 + j + user_item_view_cart_notBuy_feature_num] += 1.0 / (2 * k + 2)
                                else:
                                    user_item_feature[user_id][item_id][8 * i + 4 + j + user_item_view_cart_notBuy_feature_num] -= k / (2 * k + 2)
                user_item_feature[user_id][item_id][user_item_rate_feature_num + user_item_view_cart_notBuy_feature_num] = user_item_behavior_map[user_id][item_id][4 * 29 + 2]
                user_item_feature[user_id][item_id][user_item_rate_feature_num + user_item_view_cart_notBuy_feature_num + 1] = user_item_behavior_map[user_id][item_id][4 * 29]
                user_item_feature[user_id][item_id][user_item_rate_feature_num + user_item_view_cart_notBuy_feature_num + 2] = user_item_behavior_map[user_id][item_id][4 * 29 + 1]
    return user_item_feature, user_item_feature_num

'''
''提取用户特征
'''
def extractUserFeature(user_item_behavior_map):
    '''
    ...
    user_feature_num = 15
    user_feature = {}
    ratio = [0 for i in xrange(0, user_feature_num)]
    ...
    for user_id in user_item_behavior_map:
        ...
        for i in xrange(8, 11):
            if user_feature[user_id][i - 4] != 0:
                ...
                ratio[i] = max(ratio[i], user_feature[user_id][7] / user_feature[user_id][i - 4])
    for user_id in user_feature:
        for i in xrange(8, 11):
            if user_feature[user_id][i] == 0:
                user_feature[user_id][i] = ratio[i]
...
def extractItemFeature(user_item_behavior_map):
    ...
    item_feature_num = 14
    item_feature = {}
    ratio = [0 for i in xrange(0, item_feature_num)]
    for user_id in user_item_behavior_map:
        for item_id in user_item_behavior_map[user_id]:
    ...
    for item_id in item_feature:
        for i in xrange(8, 11):
            if item_feature[item_id][i - 8] != 0:
                ...
                ratio[i] = max(ratio[i], item_feature[item_id][i])
    ...
    for item_id in item_feature:
        for i in xrange(8, 14):
            if item_feature[item_id][i] == 0:
                item_feature[item_id][i] = ratio[i]

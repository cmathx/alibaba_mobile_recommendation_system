        ...
        for item_id in user_item_behavior_map[user_id]:
            ...
            for i in xrange(0, 29):
                if user_item_behavior_map[user_id][item_id][i * 4] != 0 and \
                    ...
                    if user_item_behavior_map[user_id][item_id][4 * 29 + 2] >= 19 or user_item_behavior_map[user_id][item_id][4 * 29 + 2] <= 0:
                ...
                for j in xrange(0, 4):
                    for k in xrange(0,  i + 1):
                        if user_item_behavior_map[user_id][item_id][4 * k + j] != 0:
                            ...
                            # else:
                            #     divisor = weight[j][6]
                            # user_item_feature[user_id][item_id][4 * i + j] = 1.0 / divisor
                            ...
                            #     divisor = weight[j][6]
                            # user_item_feature[user_id][item_id][4 * i + j] += 1.0 / divisor
                            if user_item_behavior_map[user_id][item_id][4 * 29 + 2] >= 19 and user_item_behavior_map[user_id][item_id][4*29+2] <= 00:
                                if k < 5:
                                    user_item_feature[user_id][item_id][8 * i + j + user_item_view_cart_notBuy_feature_num] += 1.0 / (2 * k + 1)
                                else:
                                    user_item_feature[user_id][item_id][8 * i + j + user_item_view_cart_notBuy_feature_num] -= k / (2 * k + 1)
                                ...
                                if k < 5:
                                    ...
                                    user_item_feature[user_id][item_id][8 * i + 4 + j + user_item_view_cart_notBuy_feature_num] -= k / (2 * k + 3)
...
def extractUserFeature(user_item_behavior_map, user_item_total_map):
    ...
    1-4:各种行为发生的天数、5-8:各种行为发生次数、9-11:各种行为转化率、12-15:行为评分、16-20:最近5天购买的次数、21-25:最近5天购买的天数
    '''
    #1-11：用户对于4种行为交互的天数，用户对于4种行为发生的次数，转化率
    #12-15：用户评分
    #16-22:用户对于4种行为发生次数，转化率（对于全部品牌）
    print 'extract user feature begin'
    user_feature_num = 15 + 5 + 5
    user_feature = {}
        ...
        for item_id in user_item_behavior_map[user_id]:
            for i in xrange(0, 5):
                for j in xrange(0, i + 1):
                    user_feature[user_id][15 + i] += user_item_behavior_map[user_id][item_id][4 * j + 3]
                    if user_item_behavior_map[user_id][item_id][4 * j + 3] != 0:
                        user_feature[user_id][20 + i] += 1.0
            for j in xrange(0, 4):
                for i in xrange(0, 29):
